        ...
        try:
            ...
            async for event in agent_loop.process(text, history, system_prompt):
                if event.type == "text_delta":
                    ...
                    self.add_output(event.data, role="assistant")
                elif event.type == "tool_call":
                    tool_name = event.data.get("name", "?")
                    tool_args = event.data.get("args", {})
                    self.add_output(
                        f"Calling tool: {tool_name}({json.dumps(tool_args, ensure_ascii=False)[:200]})",
                        role="tool",
                    )
                    ...
                    self.add_output(f"Error: {event.data}", role="error")
